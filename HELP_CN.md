...
保存的 Markdown 文档会将拖入或粘贴的图片导入文档相关的资源目录，并在 Markdown 中使用可保存的图片路径。未命名文档中的图片会暂存到应用资源库；建议先保存文档，再整理文档和图片资源的位置。

在图片模块中可以修改图片说明文字、图片地址和显示尺寸，也可以删除图片。

建议在移动 Markdown 文件时同时移动其引用的图片资源，否则图片可能无法显示。
