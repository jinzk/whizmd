...
import { addPoint, createGeometryDocument, getGeometryObject } from '../core/model'
...
describe('vertex transforms', () => {
  it('scales vertices about an anchor keeping it fixed', () => {
    ...
    expect(getGeometryObject(result, 'P1')).toMatchObject({ x: 0, y: 0 })
    expect(getGeometryObject(result, 'P2')).toMatchObject({ x: 200, y: 0 })
    expect(getGeometryObject(result, 'P3')).toMatchObject({ x: 200, y: 200 })
  })

  it('rotates vertices about a pivot preserving distances to it', () => {
    const result = rotateAboutPivot(square(), ['P1'], { x: 50, y: 50 }, Math.PI / 2)
    const p1 = getGeometryObject(result, 'P1')
  ...
  it('skips unknown vertex ids and leaves other points untouched', () => {
    ...
    const before = JSON.stringify(getGeometryObject(document, 'P5'))
    const result = transformVertices(document, ['P1', 'missing'], () => ({ x: 9, y: 9 }))
    expect(getGeometryObject(result, 'P1')).toMatchObject({ x: 9, y: 9 })
    expect(JSON.stringify(getGeometryObject(result, 'P5'))).toBe(before)
  })
})
