...
export function pickGeometryTarget(document: GeometryDocument, point: GeometryCoordinate, options: PickOptions = {}): GeometryPickTarget {
  ...
  const endpoints = [
    ...document.segments.flatMap((object) => [
    { pointId: object.start, curveId: object.id, endpoint: 'start' as const },
    { pointId: object.end, curveId: object.id, endpoint: 'end' as const },
    ]),
    ...document.curves.flatMap((object) => object.type === 'arc' ? [
      object.startAnchor ? { pointId: object.startAnchor, curveId: object.id, endpoint: 'start' as const } : null,
      object.endAnchor ? { pointId: object.endAnchor, curveId: object.id, endpoint: 'end' as const } : null
    ].filter((item): item is { pointId: string; curveId: string; endpoint: 'start' | 'end' } => Boolean(item)) : [])
  ]
  ...
  const pointHit = document.points.map((object) => ({ pointId: object.id, point: { x: object.x, y: object.y }, distance: Math.hypot(point.x - object.x, point.y - object.y) })).filter((item) => item.distance <= tolerance).sort((a, b) => a.distance - b.distance)[0]
