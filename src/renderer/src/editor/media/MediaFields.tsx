...
type Props = { alt: Field; src: Field; title: Field; href?: Field; onDelete: () => void; extra?: ReactNode }

export function MediaFields({ alt, src, title, href, onDelete, extra }: Props): React.JSX.Element {
  const { t } = useI18n()
  ...
  return <span className={`image-fields ${href ? 'image-link-fields' : ''}`}>
    ...
    <label className="image-field"><span>{t('imageSrc')}</span>{input(src, t('imageSrc'), t('enterImageAddress'))}</label>
