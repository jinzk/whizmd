...
type Props = { alt: Field; src: Field; title: Field; href?: Field; onDelete: () => void; onPickImage?: () => void; srcExtra?: ReactNode; extra?: ReactNode }

export function MediaFields({ alt, src, title, href, onDelete, onPickImage, srcExtra, extra }: Props): React.JSX.Element {
  const { t } = useI18n()
  ...
  return <span className={`image-fields ${href ? 'image-link-fields' : ''}`}>
    ...
    <label className="image-field"><span>{t('imageSrc')}</span>{input(src, t('imageSrc'), t('enterImageAddress'))}{onPickImage || srcExtra ? <span className="image-field-action">{onPickImage ? <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={onPickImage}>{t('chooseImage')}</button> : null}{srcExtra}</span> : null}</label>
